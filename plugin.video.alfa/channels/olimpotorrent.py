...
def newest(categoria):
    ...
    try:
        if categoria in ['peliculas', 'latino']:
            ...
            if itemlist and "Página" in itemlist[-1].title:
                itemlist.pop()
...
def peliculas(item):
    ...
    try:
        ...
        # Buscar enlaces a películas individuales
        patron = r'<a[^>]*href="([^"]*\/movies\/[^"\/]+\/)"[^>]*>([^<]+)</a>'
        matches = re.findall(patron, data, re.IGNORECASE)
        
        for url, title in matches:
            if not url.startswith('http'):
                url = host + url if not url.startswith('/') else host + url
...
def buscar_peliculas(item):
    ...
    try:
        ...
        patron = r'<h[0-9][^>]*><a[^>]*href="([^"]*\/movies\/[^"]*)"[^>]*>([^<]+)</a></h[0-9]>'
        matches = re.findall(patron, data, re.IGNORECASE)
